"""Safe bounded collector for the approved ANUSA Student Assistance registry."""
...
from askanu_scraper.sources.support.discovery import SupportDiscovery, SupportDiscoveryResult
...
LISTING_URL = "https://anusa.com.au/student-assistance/"
...
class SupportCollector:
    def __init__(
        ...
        parser: SupportParser | None = None,
        *,
        ...
        self._parser = parser or SupportParser()
        self._discovery = SupportDiscovery()
    ...
    def _is_listing_url(url: str) -> bool:
        ...
        return (
            parsed.scheme == "https"
            and parsed.hostname == "anusa.com.au"
            and parsed.path.rstrip("/") == "/student-assistance"
    ...
    def _sanity(
        self,
        discovery: SupportDiscoveryResult | None,
        *,
    ...
    def run_listing(
        ...
        *,
    ...
    ) -> tuple[IngestionRun, list[CommonRecord], SupportDiscoveryResult | None]:
        ...
        def fail(
            message: str,
            discovery: SupportDiscoveryResult | None = None,
            *,
        ...
        if not self._is_listing_url(listing_url):
            return fail("Listing URL is outside the approved Support boundary")
        try:
            discovery = self._discovery.discover(
                self._fetch(listing_url), listing_url, max_details=max_details
            )
        except FetchError as exc:
            return fail(f"Registry fetch failed: {exc}")
        except (ParseError, ValueError) as exc:
            return fail(f"Registry discovery failed: {exc}")
        self._sanity(discovery)
        if discovery.approved_candidate_count == 0:
            return fail(
                "Support registry produced zero approved services",
                discovery,
                suspicious_zero=True,
            )
        if discovery.approved_candidate_count != self._frozen_count:
            return fail("Support count differs from the frozen registry", discovery)
        ...
        try:
            for candidate in discovery.candidates:
                ...
                if len(parsed) != 1:
                    return fail("Expected one Support record per category", discovery)
                record = parsed[0]
                if (
                    record.domain != Domain.SUPPORT
                    or record.source_id != SOURCE_ID
                    or record.canonical_url != candidate.url
                ):
                    return fail("Support identity/provenance validation failed", discovery)
            ...
            return fail("Unexpected Support detail parser failure", discovery)
        ...
        if duplicate_ids or duplicate_urls:
            return fail(
                "Duplicate Support records detected",
                discovery,
                duplicate_record_ids=duplicate_ids,
                duplicate_urls=duplicate_urls,
            )
...
__all__ = ["SupportCollector", "SupportParser", "LISTING_URL", "SOURCE_ID"]
