...
| 2026-09-11 | Day 7 persistence implemented for review: structured Cloud Run JSON is sufficient ingestion-run evidence; NEW/CHANGED set `index_status=PENDING` and clear `embedding_version`; UNCHANGED updates observation/status while preserving index state/version and emits no embedding signal. | Use Carmen's approved `course_program_records` schema without inventing an additional table, while preserving complete operational evidence in job logs. | Scraper persistence/tests/docs and RAG changed-record handoff. | Will | User-directed implementation; Qasim/Carmen review pending |
